...
int main(int argc, char *argv[])
{
    ...
    if (server_sock < 0)
    {
        fprintf(stderr, "[x] Socket Creation! [FAILED: %s]\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    {
        fprintf(stderr, "[x] Invalid Address: %s [FAILED: %s]\n", SERVER_IP, strerror(errno));
        close(server_sock);
        exit(EXIT_FAILURE);
    }
    ...
    if (connect(server_sock, (struct sockaddr *)&server_addr, server_addr_len) <
        0)
    {
        fprintf(stderr, "[x] Connect! [FAILED: %s]\n", strerror(errno));
        close(server_sock);
        exit(EXIT_FAILURE);
    }
    ...
    if (bytes_received < 0)
    {
        fprintf(stderr, "[x] Receive! [FAILED: %s]\n", strerror(errno));
        close(server_sock);
        exit(EXIT_FAILURE);
    }
